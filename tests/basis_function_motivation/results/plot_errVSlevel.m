...
xlab={'SG Level (Adaptivity 1e-5)'};
ylab={'Relative Error'};
xVal=[7,6,5,4,3,2];

for f = 1:6
figure 
semilogy(xVal,errorHDMRmax(:,f),'--or'); hold on;
semilogy(xVal,errorHDMRavgL2(:,f),'-or');  hold on;
%semilogy(xVal,errorHDMRmin(:,f),'-.or'); hold on;

semilogy(xVal,errorSGmax(:,f),'--*b');   hold on;
semilogy(xVal,errorSGavgL2(:,f),'-*b');    hold on;
%semilogy(xVal,errorSGmin(:,f),'-.*b');
